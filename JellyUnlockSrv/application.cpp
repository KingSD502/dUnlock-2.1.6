...
bool sortApps(const QString& app1, const QString& app2)
{
    QString t1=app1;
    t1=t1.replace("app","");
    QString t2=app2;
    t2=t2.replace("app","");
    qDebug()<<t1<<t2<<t1.toInt()<<t2.toInt();
    return t1.toInt()<t2.toInt(); // если будет сортировать наоборот - поменять знак
}
...
qSort(sets_apps.begin(),sets_apps.end(),sortApps);
qDebug()<<sets_apps;
...
QString Application::GetDateDrop()
{
QDateTime t=QDateTime::currentDateTime();
QString result=t.toString("d MMMM yyyy");//d.longDayName(d.dayOfWeek())+", "+QString::number(d.day())+" "+d.longMonthName(d.month(),QDate::StandaloneFormat)+" "+QString::number(d.year());
return result;
}
